...
\ board allows it, the signal phase will be reversed and a few 탎 later
...
\ to 64탎. Call ticks with a "negate"d value. The minimum watchdog time is
...
: handle-shortcut ( -- )
    ...
    \ If shortcut is gone after 64탎, exit
    64탎 short-detected low? if exit then
